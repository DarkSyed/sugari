import { 
  ...
  export const getHealthDataForDateRange = async (startDate: Date, endDate: Date) => {
    try {
      const startTime = startDate.getTime();
      const endTime = endDate.getTime();
      
      // Get all data types for the specified date range
      const bloodSugarReadings = await getBloodSugarReadingsForTimeRange(startTime, endTime);
  ...
  export const calculateFoodStats = (entries: FoodEntry[]) => {
    ...
    const entriesWithCarbs = entries.filter(entry => entry.carbs !== null && entry.carbs !== undefined);
    ...
    const totalCarbs = entriesWithCarbs.reduce((acc, entry) => acc + (entry.carbs || 0), 0);
