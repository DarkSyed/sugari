...
const AddReminderScreen: React.FC<AddReminderProps> = ({ route }) => {
  ...
  const [title, setTitle] = useState(
    editingReminder?.title || "Check Blood Sugar",
  );
  const [body, setBody] = useState(
    editingReminder?.body || "Time to check your blood sugar!",
  );
  ...
  const handleSaveReminder = async () => {
    try {
      ...
      Alert.alert(
        "Success",
        isEditing ? "Reminder has been updated!" : "Reminder has been set!",
